...
legend("bottomright",legend = unique(names(conditNums)),col=1:4)
...
for(i in 1:length(resList)){
  ...
  print(res[order(res$padj,decreasing = F),])
  std.heatmap(log(ambrosiMatNorm[rownames(res[order(res$padj,decreasing = F),])[1:25],]+1,2))
}

```

The data for the Ingraham lab RNAseq was also passed to Salmon after trimming with trimgalore and 

```{r IngrahamBone}
datapath <- "~/code/IngrahamLab/BoneSalmonOutputs/"
setwd(datapath)
fileList <- dir()
...
std.heatmap(cor(rn.merge(boneMatNorm,ambrosiMatNorm),method = "spe"))
...
plot(res$log2FoldChange,-log(res$padj),ylab="-logPadj",x="logFC")
...
overlaps <- lapply(1:2,function(u){
  sapply(ambrosiUpDown,function(x){
    c(sum(boneUpDown[[u]]%in%x[[1]]), sum(boneUpDown[[u]]%in%x[[2]]))
  })
})
#Up
print(overlaps[[1]])
